                ...
                # do retries
                    ...
                    while self.api_counter < self.limit:
                        try:
                            ...
                            result = await func(*args, **kwargs)
                            return result
                        except (AsksException, KrakenAPIError) as err:
...
class KrakenAPI(object):
    ...
    retry : float, optional (default=.5)
        Sleep for ``retry`` seconds after an AsksException/KrakenAPIError occurred
        and retry the query until it is succesful (or the call rate limiter was
        triggered). If ``retry`` is set to 0, raise a potential
        AsksException/KrakenAPIError instead of retrying the query.
    ...
    async def get_server_time(self):
        ...
        # query
        res = await self.api.query_public('Time')
...
if __name__ == '__main__':
    ...
    dt, unixtime = trio.run(kraken.get_server_time)
